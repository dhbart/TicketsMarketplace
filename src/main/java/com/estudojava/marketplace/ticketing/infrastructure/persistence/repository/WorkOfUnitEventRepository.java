...
import com.estudojava.marketplace.ticketing.domain.*;
...
public class WorkOfUnitEventRepository implements EventRepository {
    ...
    public void save(Event event) {
        ...
        var sectors = event.getSeats().entrySet().stream()
                .map(entry -> {
                    ...
                    var seats = domainSeats.stream()
                            .map(s -> new com.estudojava.marketplace.ticketing.infrastructure.persistence.entity.Seat(
                                    s.getId(),
                                    s.getCorrelationId().id()
                            ))
                            .toList();

                    return new com.estudojava.marketplace.ticketing.infrastructure.persistence.entity.Sector(
                            domainSector.getId(),
        ...
        var entity = new com.estudojava.marketplace.ticketing.infrastructure.persistence.entity.Event(
                event.getId(),
