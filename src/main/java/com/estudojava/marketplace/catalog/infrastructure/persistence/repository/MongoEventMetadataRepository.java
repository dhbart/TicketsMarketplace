...
import com.estudojava.marketplace.catalog.domain.*;
...
public class MongoEventMetadataRepository implements EventMetadataRepository {
    ...
    private static EventMetadata mapper(
            com.estudojava.marketplace.catalog.infrastructure.persistence.entity.EventMetadata eventMetadata) {
    ...
    private static Seat mapper(
            com.estudojava.marketplace.catalog.infrastructure.persistence.entity.EventMetadata.Seat seat) {

        return new Seat(
                new SeatId(seat.getCode()),
                new SectorId(seat.getSectorName())
        );
    }

    private static Sector mapper(
            com.estudojava.marketplace.catalog.infrastructure.persistence.entity.EventMetadata.Sector sector) {

        return new Sector(
                new SectorId(sector.getName()),
                sector.getPrice()
        );
    }
